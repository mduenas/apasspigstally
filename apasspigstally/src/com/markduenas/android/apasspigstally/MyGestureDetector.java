...
public class MyGestureDetector extends SimpleOnGestureListener {
				...
				try { 
					...
					if(e1.getX() - e2.getX() > SWIPE_MIN_DISTANCE && Math.abs(velocityX) > SWIPE_THRESHOLD_VELOCITY) {
						if(canFlipRight()){
							...
							((apasspigstally)appContext).setNextPiggy();
						} else { 
							return false;
						}
					//left to right swipe
						...
						if(canFlipLeft()){
							...
							((apasspigstally)appContext).setPreviousPiggy();
						} else {
							return false;
						}
					} 
		...
		private boolean canFlipLeft() {
			return ((apasspigstally)appContext).canFlipLeft();
		}

		private boolean canFlipRight() {
			return ((apasspigstally)appContext).canFlipRight();
		} 
}
